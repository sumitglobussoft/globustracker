...
import ranktracker.crawler.vimeo.WebRankVimeo;
 ...
 * @author user
 */
public class ThreadCreator_Vimeo extends Thread {

    static Logger l = Logger.getLogger(ThreadCreator_Vimeo.class.getName());
    /**
    ...
    public ThreadCreator_Vimeo(ApplicationContext appContext) {
     ...
     *
     */
    public ThreadCreator_Vimeo(Set<String> setDomains, List<Serpkeywords> lstKeywords, ApplicationContext appContext) {
    ...
    public void run() {
        ...
        Thread[] threadWebRankVimeo = new Thread[10];
        ...
        for (int iCountz = 0; iCountz <= 9; iCountz++) {
            ...
            try {
                threadWebRankVimeo[iCountz] = new Thread(new WebRankVimeo(iCountz, lstKeywords, appContext, startTrackId, endtrackId));
                threadWebRankVimeo[iCountz].start();
            } catch (Exception e) {
        ...
        for (int iCountz = 0; iCountz <= 9; iCountz++) {
            try {
                threadWebRankVimeo[iCountz].join();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
