...
//    }
	...
	/**
	 * this method runs only when a new keyword is entered. It fetches data from SEM Rush for that keyword.
	 *
	 * @param keywrd
	 * @param customerID
	 * @param keyid
...
//                }
	...
	public int refreshGoogleKeyword(Integer keywordId) {
		...
		Serpkeywords objSerpkeywords = (Serpkeywords) session.get(Serpkeywords.class, keywordId);
		...
		if ("-".equals(objSerpkeywords.getRankGoogleRefresherDate())) {
			return 0;
		} else if (currentDate[0].equalsIgnoreCase(refreshStatus[0])) {
			...
			session.update(objSerpkeywords);
